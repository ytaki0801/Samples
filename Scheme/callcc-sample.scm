...
; 作り出されたcfuncは(lambda (x) (/ (* x 2) 4)であり，
...
; #call-with-current-continuationの利用例

; ##大域脱出
; 手続き定義のlambdaの直後にcall/ccを設定することで『何もない継続処理』を作り出し，
; 不具合が出た時などに『何もない継続処理』を行わせることで，処理全体を終了できる．
...
; 手続き冒頭に設定したcall/ccの（何も行わない）継続処理を用いることで，
